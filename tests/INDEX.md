...
| `runner_c_worker_harness` | Baseline | Proves `pw-probe-runner` (the C worker from RUNNER-RESHAPE-PLAN R5/R6/R7/R8) in isolation: a host-side C harness drives the worker through `(allow default)` happy path, bare `(deny default)` isolation, and clean exit-byte teardown. Pins the bug-report scenario the pre-Step-5 Swift worker died on — under `(deny default)` the C worker still fires applied/done sentinels, reports the kernel's EPERM via the slot, and _exit(0)s without harness SIGKILL. The runner host doesn't yet invoke this binary in production; Step 6 flips production traffic. | Built app + harness | Compiles `harness.c` once per suite run into `tests/out/.../harness.runner_c_worker` |
