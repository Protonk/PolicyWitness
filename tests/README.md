...
| `shell_helpers` | Baseline | Shell case helpers retain arguments, logs and case identity; failures stop case stages. Wrapper groups preserve child order, streams, and failure status while continuing later children | Bash + Python 3 | — | Independent receipts and subprocess observations; covers case failures, wrapper phase gates/cleanup, and explicit skips. No app or toolchain; BYOXPC controls use simulated children. |
