...
usage:
  tests/run.sh --all
  tests/run.sh --suite <preflight|unit|integration|runner_apply_isolation_v2|runner_apply_isolation_v3|runner_sandbox_denied|runner_debuggable|runner_byoxpc|smoke|blackbox_menagerie|blackbox_e2e|anomalies> [--suite <name> ...]
...
if [[ ${#suites[@]} -eq 0 ]]; then
  suites=(preflight unit integration runner_apply_isolation_v2 runner_apply_isolation_v3 runner_sandbox_denied runner_debuggable anomalies)
fi
