# runner_apply_isolation_v2

Positive smoke test for the unsandboxed-host / sandboxed-worker
architecture under an SBPL v2 `(deny default)` policy that pre-allows
the syscalls the C worker needs after apply (writing slot results to
shared memory and flipping the `done` sentinel).
...
- `tests/out/suites/runner_apply_isolation_v2/<test_id>/artifacts/*`

## Run

```
./tests/run.sh --suite runner_apply_isolation_v2
```
