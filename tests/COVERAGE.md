...
| `validator_no_reply` | host classifier (`verdictReadFailed` / `probeWriteFailed`) | none — not seam-reachable | Fires on a real I/O fault reading the validator's stdout or writing probes to its stdin (not a clean short exit — that routes to `validator_unavailable`). The `validator_executable_path` seam can't force it: a cooperating stub that drains stdin and clean-exits produces `validator_unavailable`, and the only other triggers are a genuine read/write error or the 30s read deadline, neither of which has a `_test_overrides` seam. Would need a validator-read-timeout override or `ValidatorClient`-level unit coverage. |
