...
to the `sysctl_name` filter kind on `sysctl-read`. Empirical
verification (see
...
`step.sandbox_check.outcome = "prediction_unavailable"`. Cross-check
mirrors. Attempt remains the reliable channel.
...
- The attempt slot runs unchanged.
