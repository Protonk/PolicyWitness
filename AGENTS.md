...
- Note (sandboxed harnesses): If the runner itself is sandboxed by the host (common in agent harnesses), XPC service lookup can fail with NSCocoaErrorDomain 4099 / error 159 "Sandbox restriction". This is a nested-sandbox limitation ("cannot open a sandbox inside a sandbox"), not a PolicyWitness bug. Treat it as a harness artifact and avoid drawing runtime policy conclusions from these failures. The exact harness sandbox “profile name” is often not observable; prefer operational evidence (“mach-lookup denied for <name>”) over named-profile claims.
  - Preflight: run `tools/pwlab/pw-lab inside --pw PolicyWitness.app/Contents/MacOS/policy-witness --profile minimal` before XPC work. If `inside=true`, request approval/escalation and re-run once outside the harness sandbox.
  - Default drift/noise procedure: run the fixed diagnostic sweep (`tools/pwlab/pw-lab sweep ...`) to produce per-run capsules, then replay a capsule N times (`tools/pwlab/pw-lab replay -n 25 <run_dir>`) and classify stability via witness digests rather than raw stderr/stdout.
