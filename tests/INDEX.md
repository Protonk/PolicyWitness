...
| `validator_batch_mode` | Baseline | Pins the `sb_api_validator --batch <pid>` NDJSON-over-stdin/stdout contract: 6 mixed-filter probes (3 allow + 1 bad_filter + 2 parse_error) yield 6 verdicts; per-probe failures don't abort the run; step_id is preserved when known. The C probe-runner (RUNNER-RESHAPE-PLAN Step 5) will use this shape. Per-probe CLI mode preserved unchanged. | Built app | |
