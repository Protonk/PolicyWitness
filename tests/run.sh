...
if [[ ${#suites[@]} -eq 0 ]]; then
  suites=(preflight source_drift unit integration runner_unit runner_apply_isolation_v2 runner_apply_isolation_v3 runner_sandbox_denied runner_outcome_libsandbox_unavailable runner_outcome_worker_spawn_failed runner_outcome_runner_timeout runner_debuggable anomalies)
fi
