...
| `dispatcher` | Baseline | Requested suite execution, case reports, and lifecycle events determine the same shell exit status and `run.json.ok` | Bash + Python 3 | — | Separate selectable reconciliation, accounting, and selection controls. Real dispatcher in fixture repositories; covers crashes, missing/invalid/contradictory evidence, explicit skips, wrapper aliases, and executable receipts from two usable stub apps. No app or toolchain. |
...
| `runner_outcome_runner_timeout` | Baseline | `_test_overrides.worker_timeout_ms=2000` plus `_test_overrides.worker_post_apply_hang_ms=8000` makes the C worker hang past the host deadline; the host SIGKILLs it and reports `normalized_outcome="runner_timeout"` | Built app + XPC | — | Runs ~2s wall-clock. Asserts `term_signal=9` (host-issued) and that wall-clock elapsed time matches the host deadline, not the worker's natural sleep |
