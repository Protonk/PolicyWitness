...
| `runner_unit` | Baseline | Swift runner internals: `CWorkerOrchestrator` envelope invariants, `computeDrift` truth table, `classify` worker/validator→normalized-outcome table, `buildAttemptResult` (kind, action, slot)→attempt-outcome table, prediction_unavailable host-mirror, and CWorker + ValidatorClient drivers. `HostOutcomeClassifierTests` exercises the production classifier with constructed results. Separate `SandboxApplyTests` checks exercise the unused Swift apply helper; they do not establish C-worker or CLI coverage. | `swift` on PATH | — | `tests/out/suites/runner_unit/.../pwrunner_core_tests.log`. Built via `runner/Package.swift`. |
...
| `runner_ready_byte_resilience` | Baseline | `_test_overrides.worker_pre_ready_hang_ms=2000` makes the C worker write its pre-apply ready byte after the host's 1000ms `readyByteTimeout` has closed `--ready-fd`. The worker must survive that SIGPIPE-prone write (SIGPIPE is ignored), still `sandbox_apply`, and score the probe — `normalized_outcome="ok"`. Regression guard for the `com.apple.WebProcess` slow-compile SIGPIPE bug. | Built app + XPC | — | Runs ~2-3s. Asserts `term_signal=null`, `exit_code=0`, validator ran, and the override is mirrored back. Pre-fix worker fails here with `sandbox_apply_failed`/`term_signal=13` |
