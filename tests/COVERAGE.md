...
| `xpc_error` | client (`pw-runner-client`) synthetic reply | `smoke/runner_caller_auth` (real rejected and missing-service calls); `runner_unit` response-default controls | Actual client-generated errors and successful peer replies use response 6. The caller-auth suite preserves no-effect controls and verifies disposable app copies without altering the selected source app. |
