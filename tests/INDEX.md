...
- `runner_abi_layout`: skip when `xcrun clang` or the macOS SDK is unavailable. No app-bundle dependency.
...
- `witness_contract`: off the default battery (the suite is intentionally permissive about environment shape); run on demand. `happy_path_baseline` is the regression sentinel and should always pass.
