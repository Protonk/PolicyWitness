...
| `runner_use_c_worker` | Baseline | End-to-end coverage of `_test_overrides.use_c_worker=true` (Step 6.8a). Drives a real specimen through `controller → XPC service → CWorkerOrchestrator → pw-probe-runner + sb_api_validator --batch` and asserts the v4 envelope shape: `validator_subprocess` populated, `steps[].drift` computed from validator-vs-attempt comparison, `prediction_unavailable` (op,filter) pairs skip the validator and synthesize the verdict locally, the bug-report `(deny default)` shape survives. Default path remains on the Swift worker until Step 6.8b flips. | Built app + XPC | |
