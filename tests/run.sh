...
if [[ ${#suites[@]} -eq 0 ]]; then
  suites=(preflight source_drift unit integration runner_unit runner_apply_isolation_v2 runner_apply_isolation_v3 runner_outcome_libsandbox_unavailable runner_outcome_worker_spawn_failed runner_outcome_runner_timeout runner_outcome_bad_request runner_ready_byte_resilience runner_filter_iokit_registry_entry_class runner_filter_iokit_user_client_class runner_filter_sysctl_name validator_batch_mode runner_abi_layout runner_c_worker_harness runner_use_c_worker runner_mach_service_liveness sbpl_allowdeny_consistency runner_live_worker_identity runner_exec_dac exec_fixture runner_exec_lifecycle runner_exec_inheritance)
fi
