...
| `sandbox_apply_failed` | Legacy/reserved; current ABI cannot justify a precise native apply classification | `runner_unit` / `HostOutcomeClassifierTests`; `runner_c_worker_harness/compile_failure` protects real publication separately | Current producers use `runner_failed` for published legacy preparation/application status. Unpublished storage supplies no result. Unused `SandboxApplyTests` has no production attribution credit. |
...
| `runner_timeout` | host classifier | `runner_outcome_runner_timeout`, `runner_use_c_worker/worker_timeout_ms_honored`, `witness_contract/worker_post_apply_hang_seam` | Deliberate empty-plan control plus completed single-write and mixed allowed/denied-write plans. All assert host-kill timeout evidence and both overrides; populated plans independently check file effects and retained step evidence with `drift=false` and `partial_steps=false`. |
| `runner_failed` | host classifier | `runner_unit` / `HostOutcomeClassifierTests`, `CWorkerLifecycleTests`, `CWorkerTests`; `witness_contract/worker_termination_and_log_correlation` | Published legacy failure, inconsistent/incomplete report, abnormal or unconfirmed disposition, cleanup/wait fault, or host setup/transport failure. Completed slots survive; the underlying cause can remain unknown. |
...
| `xpc_error` | client (`pw-runner-client`) synthetic reply | `smoke/runner_caller_auth` (real rejected and missing-service calls); `runner_unit` response-default controls | Actual client-generated errors and successful peer replies use response 5. The caller-auth suite preserves no-effect controls and verifies disposable app copies without altering the selected source app. |
...
response-5 mapping. Rust correlation tests and the signed CLI retain log
...
| `not_run_worker_died` | host step builder | `witness_contract/pre_apply_failure_reports_no_policy_verdict`; `runner_unit` / `AttemptOutcomeMappingTests` | Compatibility spelling for no completed attempt result. Missing/incomplete publication does not prove the operation never started; errno and drift remain null. |
