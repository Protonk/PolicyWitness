...
| `runner_filter_iokit_user_client_class` | Baseline | `iokit_user_client_class` filter on `iokit-open-service`. Same `prediction_unavailable` contract; complements `runner_filter_iokit_registry_entry_class` to cover both registry-entry and user-client class matching modes. | Built app + XPC | |
